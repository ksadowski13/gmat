...
class MutualMultiAttentionHead(nn.Module):
    def __init__(
        ...
        self._node_query_linear_1 = LinearLayer(
            node_in_feats, num_heads * self._expansion_factor, linear_projection_activation)
        self._node_query_linear_2 = LinearLayer(
            num_heads * self._expansion_factor, num_heads, linear_projection_activation)
        self._node_key_linear_1 = LinearLayer(
            node_in_feats,
            num_heads * node_in_feats * self._expansion_factor,
        ...
        self._edge_query_linear_1 = LinearLayer(
            edge_in_feats, num_heads * self._expansion_factor, linear_projection_activation)
        self._edge_query_linear_2 = LinearLayer(
            num_heads * self._expansion_factor, num_heads, linear_projection_activation)
        self._edge_key_linear_1 = LinearLayer(
            edge_in_feats,
            num_heads * edge_in_feats * self._expansion_factor,
            linear_projection_activation,
        )
    ...
    def forward(
        ...
        node_query = self._node_query_linear_1(node_inputs)
        node_query = self._node_query_linear_2(node_query)
        node_query = node_query.view(self._num_heads, -1, 1)
        node_key = self._node_key_linear_1(node_inputs)
        node_key = self._node_key_linear_2(node_key)
        node_key = node_key.view(self._num_heads, -1, self._node_in_feats)
        ...
        edge_query = self._edge_query_linear_1(edge_inputs)
        edge_query = self._edge_query_linear_2(edge_query)
        edge_query = edge_query.view(self._num_heads, -1, 1)
        edge_key = self._edge_key_linear_1(edge_inputs)
        edge_key = self._edge_key_linear_2(edge_key)
        edge_key = edge_key.view(self._num_heads, -1, self._edge_in_feats)
        ...
        node_self_attention = self._calculate_self_attention(
            node_query, self._node_in_feats)
        edge_self_attention = self._calculate_self_attention(
            edge_query, self._edge_in_feats)
        ...
        node_message_passing = self._calculate_message_passing(
            g, node_key, node_attention_projection)
        edge_message_passing = self._calculate_message_passing(
            lg, edge_key, edge_attention_projection)
