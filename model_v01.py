...
class GraphAttentionConvolution(nn.Module):
    def __init__(
        ...
        self._activation = activation
        self._linear = LinearLayer(in_feats, in_feats, activation)
...
class MutualMultiAttentionHead(nn.Module):
    def __init__(
        ...
        self._node_convolution = GraphAttentionConvolution(node_in_feats, convolution_aggregation_type, convolution_activation)
        self._edge_convolution = GraphAttentionConvolution(edge_in_feats, convolution_aggregation_type, convolution_activation)
    ...
    def forward(
        ...
        node_outputs = self._node_convolution(g, node_value, node_attention_projection)
        edge_outputs = self._edge_convolution(lg, edge_value, edge_attention_projection)
...
class MutualAttentionTransformerLayer(nn.Module):
    def __init__(
        ...
        convolution_normalization_type: str,
        linear_embedding_normalization_type: str,
        convolution_activation: str = None,
        linear_projection_activation: str = None,
        ...
        self._embedding_pooling_type = embedding_pooling_type
        ...
        if convolution_normalization_type == 'layer':
            self._node_convolution_normalization = nn.LayerNorm(node_in_feats)
            self._edge_convolution_normalization = nn.LayerNorm(edge_in_feats)
        elif convolution_normalization_type == 'batch':
            self._node_convolution_normalization = nn.BatchNorm1d(node_in_feats)
            self._edge_convolution_normalization = nn.BatchNorm1d(edge_in_feats)
        ...
        if linear_embedding_normalization_type == 'layer':
            self._node_linear_embedding_normalization = nn.LayerNorm(node_out_feats)
            self._edge_linear_embedding_normalization = nn.LayerNorm(edge_out_feats)
        elif linear_embedding_normalization_type == 'batch':
            self._node_linear_embedding_normalization = nn.BatchNorm1d(node_out_feats)
            self._edge_linear_embedding_normalization = nn.BatchNorm1d(edge_out_feats)
    ...
    def forward(
        ...
        node_embedding = self._node_convolution_normalization(node_embedding)
        edge_embedding = self._edge_convolution_normalization(edge_embedding)

        node_embedding = self._node_linear_embedding(node_embedding)
        edge_embedding = self._edge_linear_embedding(edge_embedding)  

        node_embedding = self._node_linear_embedding_normalization(node_embedding)
        edge_embedding = self._edge_linear_embedding_normalization(edge_embedding)
